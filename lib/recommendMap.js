...
module.exports = {  
    HTML:function(cafe1, cafe2, lat, lng){
        var html = `
        <!DOCTYPE html>
        <html>
        <head>
       ...
       ` 
        ...
        while(i<cafe1.length){
            const distance = viewCafe.DIST(lat, lng, cafe1[i].cafe_latitude, cafe1[i].cafe_longitude)
            html+=`
            positions1.push({
       ...
       while(i<cafe2.length){
           const distance = viewCafe.DIST(lat, lng, cafe2[i].cafe_latitude, cafe2[i].cafe_longitude)
           html+=`
           positions2.push({
